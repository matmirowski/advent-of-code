...
int main() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        char moveBlocks[100] = {'\0'};

        // add blocks to moveBlocks array and remove them from source array
        for (int i = 1; i <= move; i++) {
            char block = moveFromArray[strlen(moveFromArray) - 1];
            moveFromArray[strlen(moveFromArray) - 1] = '\0';
            moveBlocks[i - 1] = block;
        }

        // reverse moveBlocks and add them to destination array
        strrev(moveBlocks);
        strcat(moveToArray, moveBlocks);
    }
